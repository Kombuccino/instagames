...
The selected metro composition is `/assets/generated/platform/entry-scenes/metro-sunset/camera-distance-20.png`, validated on 12 September 2026 and then corrected for stronger low-poly consistency. It advances the camera 20% toward the opposite bench while preserving the measured carriage perspective, central windows, aisle poles, door geometry and passenger placement. The far-left schoolgirl's hair and shoes use explicit planar low-poly masses rather than painted strands or smooth footwear. The reader's headset has two restrained coral Fuggy-ear fins; it remains an ordinary headset and does not reproduce the mascot face or logo. The 30% study remains an interchangeable alternate selected with `metroDistance=30`; the default stays 20% until another explicit decision.

The player reads as seated, Fuggy is absent from the bench, and the MiniFugg arm/phone layer remains the continuous foreground object. On extended screens the resting phone sits between the reader and the older traveler, enlarged for readability and centred at roughly 55% of the visible height. Wagon and hand belong to one fixed `1671 × 941` desktop source scene; its phone rig is locked at `x = 671`, `y = 47`, size `625 × 1110`, and the complete scene is uniformly cover-scaled and cropped. In portrait, the current wagon/hand asset uses one fixed legacy-authored `390 × 844` logical rig. Its `390 × 693.33` composed window and hand coordinates `x = 0`, `y = 139` remain factual source geometry until that rig is deliberately migrated. Any new production targets the canonical `390 × 850` MASTER and `390 × 710` guaranteed window. Width, height, browser and app shell can only scale and crop the complete scene; they never recompute the hand independently from the wagon. The hand has no autonomous resting sway. All eight variants are complete authored phone/hand/forearm assets rather than repaired crops. They share stable phone geometry while preserving the historical skin, garment and accessory identities. Skin uses smaller anatomical facets and clothing uses the broader tailored planes of the metro art direction. Every sleeve leaves through the bottom with a clean right margin. Extensions, mirrored raster, CSS polygons and visible wrist grafts are rejected. The live MiniFugg surface is rendered behind the exact transparent screen cutout so the authored bezel masks it cleanly. The same wide plate is cover-cropped on complete screens rather than replaced with a separately composed portrait scene. Runtime derivatives are lossless WebP with PNG fallback.

The active 20% scene uses the V3 parallax split shared by the public Home and hidden Home bis transition. The exact approved carriage geometry and lighting are preserved as one fixed alpha foreground; the separate variable sunlight plate is removed. The exterior uses a fixed full sky and sun, five evenly distributed distant building groups, four intermediate groups, a modular bridge, a moving water texture, a subtle moving sparkle texture and a stationary direct reflection. All building groups are complete opaque alpha-cut objects derived from the original skyline language. Distant groups are smaller, lighter and slower rather than translucent.

For public-entry QA, `?usr=moigod&entry=1` forces the entry scene even when the URL already contains a selected `game`. Production parallax URLs carry an explicit revision because deployed public images are cached as immutable for seven days. Every changed V3 layer set must bump that revision so desktop and mobile cannot retain different generations under the same filename.

Every moving plane is a complete scene-width ring duplicated once and translated by exactly one ring. Object spacing continues across the ring boundary, so the join is part of the normal city rhythm rather than a blended image seam. Speeds use a deliberately strong depth separation: far skyline 110 seconds, near skyline 40 seconds, bridge 29 seconds and water 22 seconds. The wagon never changes exposure. Small water highlights and the fixed direct reflection pulse gently on independent short cycles; the moving water texture contains no direct sun path. Reduced-motion freezes all loops and shimmer.

The V1/V2 experiments remain rejected: the first cut buildings into horizontal bands; the second matched edge pixels but produced implausible empty intervals and visible content discontinuities. Edge-pixel equality alone is not an acceptance test. V3 was inspected at multiple positions across the water and bridge wrap, the skyline motion, the water shimmer and the constant carriage exposure before activation. `metroDistance=30` remains the approved flat comparison plate.

The hand proves object continuity during the approach, then fades out immediately after the live extended scene has taken over. This avoids leaving enlarged fingers over the readable magazine and gameplay viewport.
...
In the stable state, the phone is an actual gameplay viewport rather than a prop: keep it at the extreme foreground on the right and use 95% of the useful height. Its uninterrupted fictional glass uses the canonical `390 × 850` MASTER and `390 × 710` guaranteed window for new production; the currently integrated phone rig may remain on its explicit legacy `390 × 844` source geometry until a dedicated retune. The shell has no camera, notch, sensor, speaker island or large chin, and the holding hand never overlaps gameplay. The open magazine dominates the remaining width. Only a thin border of desk, lamp light and bedroom may remain visible.
