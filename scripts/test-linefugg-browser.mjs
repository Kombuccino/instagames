...
try {
  for (const config of configs) {
    ...
    await page.goto('http://127.0.0.1:5175/?usr=moigod&game=linefugg')
    await page.locator('.mf-console-play').first().click()
    await page.waitForFunction(() => typeof window.render_linefugg_to_text === 'function')
    await page.waitForTimeout(600)
    const state = () => page.evaluate(() => JSON.parse(window.render_linefugg_to_text()))
    ...
    if (config.reducedMotion !== 'reduce') {
      await page.waitForFunction(() => {
        const value = JSON.parse(window.render_linefugg_to_text())
        return value.rerolling && value.dimensionSlots.includes(1) && value.dimensionSlots.includes(0)
      }, null, { timeout: 1500 })
      await capture('reroll-cascade')
    }
    await page.waitForFunction(() => !JSON.parse(window.render_linefugg_to_text()).rerolling)
    ...
    if (!touch) {
      ...
      assert.equal((await state()).validateAppearance, 'hover')
      assert.equal((await state()).finished, false, 'Hover is not submission')
      await capture('validate-hover')
      await page.mouse.move(0, 0)
      await page.waitForTimeout(120)
      assert.equal((await state()).validateAppearance, 'ready')
    }
    ...
    await page.getByRole('button', { name: 'Exit game and return to cover', exact: true }).click()
    await page.waitForTimeout(600)
    ...
    assert.equal(await page.evaluate(() => typeof window.render_linefugg_to_text), 'undefined', 'Destroyed scene removes its debug hook')
