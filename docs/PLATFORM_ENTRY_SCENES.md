...
The active 20% scene has an authored parallax production split. The carriage, passengers, poles, doors and window frames form one fixed transparent foreground plate. Behind it, a separate outdoor panorama is presented through three independently clipped motion bands: sky/sun at the slowest rate, skyline at the middle rate, water/reflections at the fastest rate. The moving floor reflection remains an interior light response. The exterior motion stays restrained so it reads as a moving train rather than a sliding backdrop, and `prefers-reduced-motion` freezes every band. `metroDistance=30` remains the approved flat comparison plate until it receives the same production split.
