...
The active 20% scene uses an object-cutout parallax split shared by the public Home entry and the hidden Home bis transition. The fixed backdrop is a full-frame sky with a fixed sun. In front of it, complete alpha-cut silhouettes form a slow far skyline, a faster near skyline and a faster shore/bridge layer; a separate water base moves fastest. Every moving layer travels in one direction and uses a normalized seamless horizontal loop. Deeper layers always move more slowly. Horizontal panorama bands that cut through buildings are explicitly obsolete.

The carriage, passengers, poles, doors and window frames remain one geometrically exact fixed plate with only ambient light. Direct golden light is isolated in a second transparent carriage plate. The direct reflection below the fixed sun is also a separate stationary plate above the moving water. One synchronized 48-second occlusion curve dims the carriage sunlight and water reflection whenever a near building crosses the sun, while the sun and light angle remain fixed. `prefers-reduced-motion` freezes all movement and exposure changes. `metroDistance=30` remains the approved flat comparison plate until it receives the same production split.
