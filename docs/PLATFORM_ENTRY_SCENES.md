...
The active public Home and hidden Home bis transition use the approved flat 20% plate. The parallax experiments from 12–13 September are rejected: the first cut buildings into horizontal bands; the second matched edge pixels but produced implausible empty intervals and visible content discontinuities. Neither may be restored as production code.

The future parallax still requires a fixed sky and sun, complete alpha-cut depth layers, separate water and direct reflection, and separate ambient/direct carriage lighting. Its source layers must first be authored as genuinely repeating scenes whose object distribution is continuous across the join. Edge-pixel equality alone is not an acceptance test. Until a full-cycle preview is visually approved, the flat authored plate remains canonical. `metroDistance=30` remains the approved flat comparison plate.
