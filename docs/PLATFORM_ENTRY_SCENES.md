...
The player reads as seated, Fuggy is absent from the bench, and the existing MiniFugg arm/phone layer remains the continuous foreground object. On extended screens the resting phone sits between the reader and the older traveler. All eight arm variants now use extended production PNGs: their original hand, phone, skin and accessories are retained while a faceted sleeve continuation exits the right viewport. This continuation belongs to the asset itself; CSS polygons and duplicated background images are rejected. The same wide plate is cover-cropped on complete screens rather than replaced with a separately composed portrait scene. The current runtime derivative is lossless WebP with a PNG fallback. A restrained moving floor reflection provides the first ambient production slice. Genuine three-plane parallax still requires separate foreground player/poles, middle carriage/passengers and exterior-city assets; the flattened approved plate is not treated as if it already contained those layers.
