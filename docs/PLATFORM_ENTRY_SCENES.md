...
Every moving plane is a complete scene-width ring duplicated once and translated by exactly one ring. Object spacing continues across the ring boundary, so the join is part of the normal city rhythm rather than a blended image seam. Speeds are depth ordered: far skyline 82 seconds, near skyline 49 seconds, bridge 31 seconds and water 22 seconds. A synchronized 12.25-second cycle lowers the direct carriage light and fixed water reflection as intermediate buildings cross the fixed sun. Reduced-motion freezes all loops and lighting changes.
