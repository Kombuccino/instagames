...
try {
  ...
  for (const format of formats) {
    ...
    assert.match(await idleFrame.evaluate(element => getComputedStyle(element).filter), /brightness\(0\.16\)/,
      'PLAY must use a genuinely dark off state, not the low-intensity source appearance')
