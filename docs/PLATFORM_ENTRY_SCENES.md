...
The player reads as seated, Fuggy is absent from the bench, and the existing MiniFugg arm/phone layer remains the continuous foreground object. On extended screens the resting phone sits between the reader and the older traveler. All eight arm variants use extended production PNGs: the original 941-pixel-wide hand, phone, skin, wrist, accessory and sleeve region is preserved exactly. Each forearm continues at its existing angle with restrained facets sampled from its own clothing, keeps a credible width and exits through the bottom instead of widening toward the right edge. A mirrored extension, shared recoloured sleeve, CSS polygon or duplicate of another arm is rejected. The same wide plate is cover-cropped on complete screens rather than replaced with a separately composed portrait scene. Runtime derivatives are lossless WebP with PNG fallback.
