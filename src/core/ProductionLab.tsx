...
const SCREEN_X: Record<StateId, number> = { covers: 220, proto: 1650, da: 4450, release: 8300 }
...
function buildLineFuggPlan(game: InstagameDefinition): PlanProject {
  ...
  const screens: PlanScreen[] = [
    ...
    },
    {
      ...
      source: 'DA validée · référence globale uniquement', x: SCREEN_X.da, y: 520, image: `${LINEFUGG_REBIRTH_DA_ROOT}/linefugg-rebirth-editorial-paper-lab-390x850-r2.webp`, preview: 'proto', anchor: 'center',
    },
  ]
  ...
  return {
    ...
    summary: 'Rebirth repart du vrai LineFugg classique. Le Proto reste la vérité fonctionnelle ; la DA validée est maintenant traitée comme un inventaire visuel complet avant toute production d’assets. Release reste vide tant que cette planche n’est pas validée.',
    screens,
    nodes,
  }
}
  ...
  return <main className="mfpl">
    ...
    <div className={`mfpl-layout ${selection ? 'has-inspector' : ''}`}>
      ...
      >
        ...
        <div className="mfpl-world" style={{ width: worldWidth, height: WORLD_HEIGHT, transform: `translate(${camera.x}px, ${camera.y}px) scale(${camera.zoom})` }}>
          {displayZones.map((zone) => <div key={zone.id} className={`mfpl-zone is-${zone.id}`} style={{ left: zone.x, width: zone.width, height: WORLD_HEIGHT }}><div className="mfpl-state-title"><b>{STATE_LABEL[zone.id]}</b><span>{zone.subtitle}</span>{!project.screens.some((screen) => screen.state === zone.id) && <em>vide pour le moment</em>}</div></div>)}

          {viewMode === 'exploded' && <svg className="mfpl-links mfpl-links-underlay" width={worldWidth} height={WORLD_HEIGHT}>
          ...
          {viewMode === 'exploded' && <svg className="mfpl-links mfpl-links-foreground" width={worldWidth} height={WORLD_HEIGHT}>
