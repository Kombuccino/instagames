...
The player reads as seated, Fuggy is absent from the bench, and the existing MiniFugg arm/phone layer remains the continuous foreground object. On extended screens the resting phone sits between the reader and the older traveler; portrait uses the same visual interval instead of drifting left. All eight arm variants use extended production PNGs: the original 941-pixel-wide hand, phone, skin, wrist, accessory and sleeve region is preserved exactly. Each forearm continues its own raster texture at its existing angle, keeps a credible width and exits through the bottom instead of widening toward the right edge. A visible change of facet scale or palette, shared recoloured sleeve, CSS polygon or duplicate of another arm is rejected. The same wide plate is cover-cropped on complete screens rather than replaced with a separately composed portrait scene. Runtime derivatives are lossless WebP with PNG fallback.
