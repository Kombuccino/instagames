...
The active 20% scene has an authored parallax production split used by both the public Home entry and the hidden Home bis transition. The carriage, passengers, poles, doors and window frames form one fixed transparent foreground plate. Behind it, a separate outdoor panorama is presented through three independently clipped bands: sky/sun fixed at this scale, skyline moving at the middle rate, water/reflections moving faster. Buildings periodically occlude the distant sun: one shared 13-second timing curve briefly lowers carriage exposure, the fixed floor-light intensity and water-reflection brightness, without moving the light angle. The exterior motion stays restrained so it reads as a moving train rather than a sliding backdrop, and `prefers-reduced-motion` freezes every band and exposure change. `metroDistance=30` remains the approved flat comparison plate until it receives the same production split.
