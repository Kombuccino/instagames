...
import './VladsSkewers.runtime-effects.css'
import './VladsSkewers.delivery.css'
import './VladsSkewers.grill-gore.css'
import './VladsSkewers.progression.css'
...
export function VladsSkewers({ active, seed, restartToken, session }: GameComponentProps) {
  ...
  return <div ref={rootRef} className={`vlad-game ${view.slowTimer > 0 ? 'is-slowed' : ''} ${orderReady ? 'is-ready-to-serve' : ''}`}>
    ...
    {levelCard > 0 && <div className="vlad-level-card vlad-level-card-banner" key={`${restartToken}-${levelCard}`} aria-hidden="true">
