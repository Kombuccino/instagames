...
The player reads as seated, Fuggy is absent from the bench, and the existing MiniFugg arm/phone layer remains the continuous foreground object. On extended screens the resting phone sits between the reader and the older traveler. All eight arm variants use extended production PNGs: the original 941-pixel-wide hand, phone, skin, wrist, accessory and sleeve region is preserved exactly, and the remaining sleeve is continued from that variant's own raster facets until it exits the right viewport. A shared recoloured sleeve, CSS polygon or duplicate of another arm is rejected. The same wide plate is cover-cropped on complete screens rather than replaced with a separately composed portrait scene. Runtime derivatives are lossless WebP with PNG fallback.

The active 20% scene uses the V3 parallax split shared by the public Home and hidden Home bis transition. The exact approved carriage geometry is preserved as an alpha foreground source, then separated into a visibly shaded ambient base and a strong direct-sun plate; the ambient plate must not retain the bright floor and passenger highlights by itself. The exterior uses a fixed full sky and sun, five evenly distributed distant building groups, four intermediate groups, a modular bridge, a moving water texture and a stationary direct reflection. All building groups are complete alpha-cut objects derived from the original skyline language.
